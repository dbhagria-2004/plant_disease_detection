...
import time
def class_list():
    class_names = ['Apple___Apple_scab', 'Apple___Black_rot', 'Apple___Cedar_apple_rust', 'Apple___healthy', 'Blueberry___healthy', 'Cherry_(including_sour)___Powdery_mildew', 'Cherry_(including_sour)___healthy', 'Corn_(maize)___Cercospora_leaf_spot Gray_leaf_spot', 'Corn_(maize)___Common_rust_',
...
def model_pred(test_image):
    ...
    return list_of_classes[result_index]
...
# Home Page
if(app_mode == 'Home'):
    st.header("Plant Disease Detection Model")
...
#Prediction Page
    ...
    st.header("Predion Page for Disease Recognition")
    ...
    if(st.button("Predict")):
        with st.spinner("Model is predicting... Please wait"):
            ...
            time.sleep(5)
            st.balloons()
            prediction = model_pred(test_image)
        st.success("Model is Predicting its a:  {}".format(prediction))